...
class App:
  ...
  def ws(self, handle_str):
    if handler_str not in ['open', 'message', 'close', 'error']:
  ...
  def ws_handler(self, handler_str, *args, **kwargs):
    func = self._ws_handler.get(handler_str)
    if func:
      func(*args,**kwargs)
...
class WsContext:
  ...
  def pop(self):
    ...
    len_flag = ord(self._buffer[1]) & 0x7F
    if len_flag == 126:
...
class WsHandler:
  ...
  def wshandshake(self, conn, v):
    key = base64.b64decode(hashlib.sha1(v + '258EAFA5-E914-47DA-95CA-C5AB0DC85B11').digest())
...
class Httpy:
  ...
  def protocol(self, conn, addr, port):
    ...
    if 'Sec-WebSocket-Key' in header:
      ...
      self.httpHander.process(header, conn)
      conn.close()
